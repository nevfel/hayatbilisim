<template>
    <SiteLayout title="Hızlı Ödeme">
        <Head title="Hızlı Ödeme" />

        <div class="py-12">
            <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
                <!-- Ödeme Bilgileri -->
                <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                    <h2 class="text-2xl font-bold text-gray-900 mb-4">Ödeme Bilgileri</h2>
                    <div class="border-t border-gray-200 pt-4">
                        <div class="flex justify-between mb-2">
                            <span class="text-gray-600">Ödeme No:</span>
                            <span class="font-semibold">{{ quickPayment.payment_number }}</span>
                        </div>
                        <div class="flex justify-between mb-2" v-if="quickPayment.description">
                        ...
                        <div class="flex justify-between mb-4">
                            ...
                            <span class="text-3xl font-bold text-indigo-600">{{ formatCurrency(quickPayment.amount) }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    <h2 class="text-2xl font-bold text-gray-900 mb-6">Kredi Kartı ile Ödeme</h2>
                    ...
                    <form @submit.prevent="submitPayment">
                        ...
                        <button
                            ...
                            class="w-full bg-indigo-600 text-white py-3 px-6 rounded-md hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed font-semibold text-lg"
                        >
                            <span v-if="form.processing">İşleniyor...</span>
                            <span v-else>Ödeme Yap ({{ formatCurrency(quickPayment.amount) }})</span>
                        </button>
                    </form>
                </div>
                ...
                <div class="mt-6 text-center">
                    <p class="text-sm text-gray-500">
                        Sorun yaşıyorsanız
                        <a href="mailto:{{ quickPayment.gon_email }}" class="text-indigo-600 hover:text-indigo-700">
                            {{ quickPayment.gon_email }}
                        </a>
...
const submitPayment = () => {
    ...
    })).post(route('quick-payment.pay', props.quickPayment.payment_number));
};
</script>
